...
<body>
	...
	<%
		try {
			String name, gender, email; // 데이터베이스 안에 필드를 저장하기 위한 객체변수 선언
			int no;
			// JDBC 드라이버 설정
			Class.forName("com.mysql.jdbc.Driver");
			Connection con = DriverManager.getConnection("JDBC:mysql://localhost:3306/jsp22_b", "root", "1234");
			Statement stmt = con.createStatement();
			ResultSet rs = stmt.executeQuery("select no, name, gender, email from student");
			while(rs.next()) {
				no = rs.getInt(1); // 번호 필드 값
				name = rs.getString(2); // 이름 필드 값
				gender = rs.getString(3); // 성별 필드 값
				email = rs.getString(4); // 이메일 필드 값
	%>
	<tr>
